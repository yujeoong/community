...
<body>
	...
	<div id="ListHeader">승부예측</div>
			<select id="searchType" name="searchType"> 
				<option value="1">경기+내용</option>
		...
		<%
			if (session.getAttribute("sessionId") != null && (Integer) session.getAttribute("sessionLevel") == 2) {
		%>
		<button class="btn btn-outline-primary pull-right" onclick="location.href='writeForm_p?categoryId=1'" id="btnWrite">글쓰기</button>
		<%
			}
		%>
...
<script>
	...
	function drawList(list) { //데이터가 정상적으로 들어와있고 로그인이 되어있는 상태면 위의 drawList(data.list)를 호출한다.	
		...
		for (var i = 0; i < list.length; i++) {
			...
			content += '<td style="text-align:center">' + endDate.toLocaleString('ko-KR', options) + '</td>';  

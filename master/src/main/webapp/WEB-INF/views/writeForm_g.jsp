	...
	<div class="container">
		...
		<div class="pull-left">
			<c:if test="${categoryId eq 4}">
				<div id="ListHeader">자유게시판 글 작성</div>
			</c:if>
			<c:if test="${categoryId eq 6}">
				<div id="ListHeader">공지사항 글 작성</div>
			</c:if>
			<c:if test="${categoryId eq 7}">
				<div id="ListHeader">건의사항 글 작성</div>
			</c:if>
		</div>
		...
		<form name="form" id="form" method="POST" action="write_g" enctype="multipart/form-data">

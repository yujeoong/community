...
<body>
	...
	<table class="table table-striped">
		...
		<tbody id="list">
			...
			<button class="btn btn-outline-primary pull-right" onclick="location.href='writeForm_p?categoryId=2'" id="btnWrite">글쓰기</button>
		<% } %>
...
<script>
	...
	function drawList(list) { //데이터가 정상적으로 들어와있고 로그인이 되어있는 상태면 위의 drawList(data.list)를 호출한다.
		...
		for (var i = 0; i < list.length; i++) {
			//console.log(list[i]);
			content += '<tr>';
			content += '<td>' + list[i].postId + '</td>'; //list의 i번째에 있는 id 

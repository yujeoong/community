...
<body>
	...
	<div id="ListHeader">경매</div>
		...
		%>
		<button class="btn btn-outline-primary pull-right" onclick="location.href='writeForm_a?categoryId=5'" id="btnWrite">글작성</button>
		<%
			}
		%>
...
<script>
	...
	function drawList(list) { //데이터가 정상적으로 들어와있고 로그인이 되어있는 상태면 위의 drawList(data.list)를 호출한다.
		...
		for (var i = 0; i < list.length; i++) {
			...
			content += '<td><a href="auction_detail?postId='+list[i].postId+'">' + list[i].subject +'</a></td>';

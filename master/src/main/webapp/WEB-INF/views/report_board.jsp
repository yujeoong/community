...
<body class="container">  
	...
	<div class="col-7">
		 <span ><button style="color: skyblue">게시글 신고</button></span>
		 <span><button onclick="location.href='report_comment'">댓글 신고</button></span>
		 <table class="table table-hover">
			 ...
			 <div class="search">
				<select id="report_board_user" name="status_user" class="selectpicker">
				...
				<input type="button" value="취소" class="btn btn-secondary" onclick="location.href='report_board'" />
			</div>
	</div>
	</div>
	<script
...
$(document).on("click","#report_process",function(){
		...
		if(typeof radio == "undefined"){
			...
			location.href="Admin_report_process?reportId="+radio+"&&type="+"1";
		}
});
...
function search_user_report(page2){
	status_change.push($("#report_board_user").val());
	console.log(status_change);
	if(flag){
	flag=false;
	var user=$("#report_board_user").val();
	...
	$.ajax({
		type:"post"
		,url:"search_board_report"
		,dataType:"JSON"
		,data:{"user":user,"search":search,"page":page2}
		,success:function(data){
			listCall(data.list);
			if(pageflag == true && $('.pagination').data("twbs-pagination")
					|| status_change.at(-2) != $("#report_board_user").val()){
